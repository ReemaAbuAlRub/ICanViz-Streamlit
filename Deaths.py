...
selected= option_menu(menu_title=None,options=['Home','About','Mortality','Survivals','Risks','YLD','Login','Contact Us'],icons=['house','info-circle','graph-down','graph-up','file-medical','stopwatch','person','envelope'],orientation='horizontal',menu_icon='cast',default_index=2)
st.markdown('---')
...
st.write('\n')
st.write('\n')
st.markdown("<h1 style='text-align: center;'> Mortality </h1>", unsafe_allow_html=True)
st.write('\n')
st.write('\n')
...
def countries(country):
    ...
    fig = px.line(filtered, x="Year", y='Avg_death_per', title=f"Cancer Mortality Rate in {country} From 1990 - 2019",labels={"Avg_death_per":'Average Mortality Rate'},height=500,width=600)
...
spaces(2)
tab1,tab2=st.tabs(['Arab Countries','Cancer Sites'])
with tab1:
    spaces(2)
    st.header('Arab Countries')
    spaces(1)
...
with tab2:
    ...
    with c2:
        ans = st.selectbox( 'Select Country',(country),key=77)
